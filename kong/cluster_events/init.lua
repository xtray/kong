...
local function process_event(self, row)
  ...
  for j = 1, #cbs do
    if not row.nbf then
      ...
      local now = row.now or self.strategy:server_time() or ngx_now()
...
local function poll(self)
  ...
  for rows, err, page in self.strategy:select_interval(self.channels, min_at) do
    ...
    if page == 1 then
      local at = count > 0 and rows[1].now or self.strategy:server_time() or ngx_now()
      local ok, err = self.shm:safe_set(CURRENT_AT_KEY, at)
      if not ok then
        return nil, "failed to set 'at' in shm: " .. err
      end
    end
